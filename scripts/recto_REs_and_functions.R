...
#### modify the read.bioanalyzer function
#### to disable a data check that results
#### in data from one run not being loaded
#####
read.bioanalyzer_recto <- function(xml.file, fit = "spline") {
  ...
  assay.info <- list(file.name = xmlValue(chip.root[["Files"]][["File"]][["FileInformation"]][["FileName"]]), 
  ...
  defined.ladder.peaks <- xmlToDataFrame(chip.root[["AssayBody"]][["DAAssaySetpoints"]][["DAMAssayInfoMolecular"]][["LadderPeaks"]], 
                                         colClasses = rep("numeric", 4))
  result.list <- xmlApply(chip.root[["Files"]][["File"]][["Samples"]], 
                          function(this.sample) {
                            if (xmlValue(this.sample[["HasData"]]) != "true") 
                              NULL
                            else {
                              well.number <- as.integer(xmlValue(this.sample[["WellNumber"]]))
                              sample.name <- trimws(xmlValue(this.sample[["Name"]]))
                              is.ladder <- (trimws(xmlValue(this.sample[["Category"]])) == 
                                              "Ladder")
                              ...
                              peaks.raw <- xmlToDataFrame(this.sample[["DAResultStructures"]][["DARIntegrator"]][["Channel"]][["PeaksMolecular"]], 
                                                          stringsAsFactors = F)
                              peaks <- data.frame(peak.observations = trimws(peaks.raw$Observations), 
                                                  ...
                                                  area = as.numeric(peaks.raw$Area), concentration = as.numeric(peaks.raw$Concentration), 
                              ...
                              raw.data <- data.frame(time = as.numeric(xmlValue(signal.data[["XStart"]])) + 
                                                       as.numeric(xmlValue(signal.data[["XStep"]])) * 
                                                       (1:n.values - 1), fluorescence = readBin(base64decode(xmlValue(signal.data[["ProcessedSignal"]])), 
                                                                                                "numeric", size = 4, n = n.values, endian = "little"), 
                                                     stringsAsFactors = F)
                              which.lower.marker <- which(peaks$peak.observations == 
                                                            "Lower Marker" & peaks$concentration == defined.ladder.peaks$Concentration[1])
                              stopifnot(length(which.lower.marker) == 1)
                              if (has.upper.marker) {
                                which.upper.marker <- which(peaks$peak.observations == 
                                                              "Upper Marker" & peaks$concentration == defined.ladder.peaks$Concentration[nrow(defined.ladder.peaks)])
                                stopifnot(length(which.upper.marker) == 1)
                                alignment.coefficient <- diff(peaks$aligned.time[c(which.lower.marker, 
                                                                                   which.upper.marker)])/diff(peaks$time[c(which.lower.marker, 
                                                                                                                           which.upper.marker)])
                                alignment.offset <- peaks$aligned.time[which.lower.marker] - 
                                  alignment.coefficient * peaks$time[which.lower.marker]
                              }
                              else {
                              ...
                              raw.data$aligned.time <- raw.data$time * alignment.coefficient + 
                                alignment.offset
                              list(data = raw.data, samples = data.frame(batch, 
                                                                         well.number, sample.name, sample.observations, 
                                                                         sample.comment, RIN, is.ladder, stringsAsFactors = F), 
                                   peaks = peaks, alignment.values = c(alignment.coefficient, 
                                                                       alignment.offset))
                            }
                          })
  for (i in length(result.list):1) if (is.null(result.list[[i]])) 
    result.list[[i]] <- NULL
  result <- structure(list(data = do.call(rbind, c(lapply(1:length(result.list), 
                                                          function(i) cbind(sample.index = i, result.list[[i]]$data)), 
                                                   make.row.names = F)), assay.info = list(assay.info), 
                           samples = do.call(rbind, c(lapply(result.list, function(x) x$samples), 
                                                      make.row.names = F)), peaks = do.call(rbind, c(lapply(1:length(result.list), 
                                                                                                            function(i) if (is.null(result.list[[i]]$peaks)) NULL else cbind(sample.index = i, 
                                                                                                                                                                             result.list[[i]]$peaks)), make.row.names = F)), 
                           regions = NULL, mobility.functions = NULL), class = "electrophoresis")
  if (all(is.na(result$samples$RIN))) 
    result$samples$RIN <- NULL
  ...
  for (field in c("batch", "well.number", "sample.name", "sample.observations", 
                  "sample.comment")) result$samples[[field]] <- factor(result$samples[[field]], 
                                                                       levels = unique(result$samples[[field]]))
  result$peaks[, "peak.observations"] <- factor(result$peaks[, 
                                                             "peak.observations"])
  regions.raw <- xmlToDataFrame(chip.root[["AssayBody"]][["DASampleSetpoints"]][["DAMSmearAnalysis"]][["Channel"]][["RegionsMolecularSetpoints"]], 
                                stringsAsFactors = F)
  if (nrow(regions.raw) > 0) 
    result$regions <- data.frame(sample.index = rep(1:nrow(result$samples), 
                                                    each = nrow(regions.raw)), lower.length = as.numeric(regions.raw$StartBasePair), 
  ...
  result <- calculate.molarity(calculate.concentration(calculate.length(result, 
                                                                        fit), defined.ladder.peaks$Concentration))
  if (!is.null(result$regions)) {
    result$regions$lower.time <- NA
    result$regions$upper.time <- NA
    for (i in 1:nrow(result$samples)) {
      which.regions <- which(result$regions$sample.index == 
                               i)
      result$regions$lower.time[which.regions] <- (result$regions$lower.aligned.time[which.regions] - 
                                                     alignment.values[[i]][2])/alignment.values[[i]][1]
      result$regions$upper.time[which.regions] <- (result$regions$upper.aligned.time[which.regions] - 
                                                     alignment.values[[i]][2])/alignment.values[[i]][1]
    }
  }
  result
